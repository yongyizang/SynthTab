...
GUITAR_frefix = '_'.join(STATE.split('/')[-1].split('_')[:3])
...
def render_audio(subfolders, state, path_indir, path_outdir, extract_pitch=True, mono=True):
    ...
    for song in tqdm(subfolders, desc="Rendering"):
        ...
        outdir = os.path.join(path_outdir, GUITAR_frefix, song.split("/")[-1].strip())
...
if __name__ == '__main__':
    ...
    for folder in subfolders:
        ...
        if splits[-1] != 'Guitar':
            continue
        if splits[2] in {'Electric'} and splits[3] in {'Jazz', 'Clean'}:
            electric_folders.append(folder)

    num_subfolders = len(subfolders)
    print('num subfolders:', num_subfolders)
    num_electric_folders = len(electric_folders)
    print('num electric_folders:', num_electric_folders)

    #render_audio(electric_folders, STATE, path_indir, path_outdir, extract_pitch=True, mono=True)
    #exit()
    ...
    random.shuffle(electric_folders)

    subsubfolders = np.array_split(electric_folders, num_workers)
